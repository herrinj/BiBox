...
%
%   J(phase) = min 0.5*sum(mod_2pi(bispec_phase - A*x)^2*weights^2)
%
...
% Objective function
phase_diff = mod(bispec_phase - A*x(:) + pi,2*pi) - pi;

Jc = 0.5*sum(weights.*(phase_diff.^2));

% Gradient of objective function
if doGrad
    dJ = -(A'*(weights.*phase_diff))';
end

% Note that the Hessian does not depend on phi, thus we compute
% this once as a persistent variable and only have to compute the following
% code once
if doHess
    persistent Hper;
    if isempty(Hper)

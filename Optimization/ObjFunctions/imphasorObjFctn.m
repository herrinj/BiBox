function [Jc, para, dJ, H] = imphasorObjFctn(x, A, bispec_phase, dims, pupil_mask, varargin)
%
%   Evaluates objective function, gradient, and approximate Hessian for the
%   phase recovery objective function  
%
%   J(x) = min 0.5*sum([real(delta)^2 - imag(delta)^2]*weights) + 0.5*alpha*S(image)
%              
%   where delta = exp(i*bispec_phase) - exp(i*(A*phase(x)))and phase is the 
...
Jc = 0.5*weights'*((cosb - cosAp).^2 + (sinb -sinAp).^2);
...
if doGrad % Take the derivative of the objective function w.r.t. the image
    ...
    phase_diff = A'*(weights.*(cosb.*sinAp - sinb.*cosAp));
...
%
    ...
    fctn  = @(x) imphasorObjFctn(x, A, bispec_phase, dims, pupil_mask,'regularizer','tv','alpha',1e-1);

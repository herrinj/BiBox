function [func, para, grad, hess] = imphasor_rec(image, A, bispec_phase,  weights, ADA, pupil_mask, alpha, reg_flag, pospec)
%
...
if alpha > 0
    switch reg_flag
        ...
        case{'grad',2}
            R = getGradient([0 1 0 1],dims);
            reg_term = sum((R*image(:)).^2);
    end
end
...
if doHess % returns the full, untruncated Hessian-matrix multiplication in
    ...
    switch reg_flag
        case{'pos',0}
            ...
            hess.operator = @(pk) image_rec_hess_mult2(pk, IMAGE, ADA, pupil_mask, alpha, reg_flag, neg_inds);
            hess.flag = 'oper';
        case{'pow',1}
            %weights = spdiags(weights,0,size(A,1),size(A,1));
            hess.operator = @(pk) image_rec_hess_mult2(pk, IMAGE, ADA, pupil_mask, alpha, reg_flag, image);
            hess.flag = 'oper';
        case{'grad',2}
            %weights = spdiags(weights,0,size(A,1),size(A,1));
            hess.operator = @(pk) image_rec_hess_mult2(pk, IMAGE, ADA, pupil_mask, alpha, reg_flag, R);
            hess.flag = 'oper';
    end
end
